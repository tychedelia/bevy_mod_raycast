...
use bevy_color::palettes::css::{GRAY, GREEN, RED};
...
fn bouncing_raycast(
    ...
    bounce_ray(ray, &mut raycast, &mut gizmos, Color::from(RED));

    if let Some(cursor_ray) = **cursor_ray {
        bounce_ray(cursor_ray, &mut raycast, &mut gizmos, Color::from(GREEN))
    }
}
...
fn bounce_ray(mut ray: Ray3d, raycast: &mut Raycast, gizmos: &mut Gizmos, color: Color) {
    ...
    for i in 0..MAX_BOUNCES {
        if let Some((_, hit)) = raycast.cast_ray(ray, &RaycastSettings::default()).first() {
            ...
            intersections.push((hit.position(), Color::from(color.linear() * bright)));
            gizmos.sphere(hit.position(), Quat::IDENTITY, 0.005, Color::from(color.linear() * bright * 2.0));
...
fn setup_scene(
    ...
    let plane = PbrBundle {
        ...
        material: materials.add(Color::from(GRAY.with_alpha(0.01))),
        ..default()
    };
